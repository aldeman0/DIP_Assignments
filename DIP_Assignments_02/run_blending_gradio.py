...
def create_mask_from_points(points, img_h, img_w):
    ...
    # 使用 PIL 的 polygon 函数填充多边形区域为 255 (白色)
    if len(points) > 0:
        ImageDraw.Draw(mask_img).polygon([tuple(p) for p in points], fill=255)
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    # 调整内核形状以适配 conv2d (out_channels, in_channels, k_h, k_w)
    # 由于是 RGB 三通道独立卷积，设置 groups=3
    laplacian_kernel = laplacian_kernel.view(1, 1, 3, 3).repeat(3, 1, 1, 1)

    # 使用 conv2d 计算拉普拉斯响应，padding=1 保证输出尺寸与原图一致
    lap_fg = F.conv2d(foreground_img, laplacian_kernel, padding=1, groups=3)
    lap_blended = F.conv2d(blended_img, laplacian_kernel, padding=1, groups=3)

    # 将掩码扩展并转为布尔索引 [cite: 365, 373]
    fg_mask_bool = foreground_mask.expand(-1, 3, -1, -1).bool()
    bg_mask_bool = background_mask.expand(-1, 3, -1, -1).bool()

    # 提取掩码内的像素值并计算均方误差 (MSE)
    loss = F.mse_loss(lap_blended[bg_mask_bool], lap_fg[fg_mask_bool])
